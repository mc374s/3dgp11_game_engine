...
Primitive3D::Primitive3D(ID3D11Device *a_pDevice)
{
	...
	loadVertexShader(a_pDevice, "3dgp/primitive3d_vs.cso", layoutDesc, ARRAYSIZE(layoutDesc), &m_pVertexShader, &m_pInputLayout);

	loadPixelShader(a_pDevice, "3dgp/primitive3d_ps.cso", &m_pPixelShader);
...
void Primitive3D::initialize(ID3D11Device *a_pDevice, const int &a_type, const int &a_latitudeNum, const int &a_longitudeNum)
{
	...
	if (a_type==GEOMETRY_CYLINDER)
	{
		...
		{
			vertexCount = 0;
			vertices[m_latitudeNum * 4 * i + vertexCount].position = XMFLOAT3(0.0f, hHalf, 0.0f);
			vertices[m_latitudeNum * 4 * i + vertexCount].normal = XMFLOAT3(0.0f, 1.0f, 0.0f);
			// Top
			indices[indexCount] = m_latitudeNum * 4 * i + vertexCount;
			indexCount++;
			vertexCount++;
			// TopRight
			indices[indexCount] = m_latitudeNum * 4 * ((i + 1) % m_longitudeNum) + vertexCount;
			indexCount++;
			vertices[m_latitudeNum * 4 * i + vertexCount].position = XMFLOAT3(dHalf*cosf(i*angle)*scaleAdjust, hHalf, dHalf*sinf(i*angle)*scaleAdjust);
			vertices[m_latitudeNum * 4 * i + vertexCount].normal = XMFLOAT3(0.0f, 1.0f, 0.0f);
			// TopLeft
			indices[indexCount] = m_latitudeNum * 4 * i + vertexCount;
			indexCount++;
			...
			{
				vertices[m_latitudeNum * 4 * i + vertexCount].position = XMFLOAT3(dHalf*cosf(i*angle)*scaleAdjust, hHalf, dHalf*sinf(i*angle)*scaleAdjust);
				vertices[m_latitudeNum * 4 * i + vertexCount].normal = XMFLOAT3(cosf(i*angle + angleHalf), tan(angleY), sinf(i*angle + angleHalf));
				// LeftTop
				indices[indexCount] = m_latitudeNum * 4 * i + vertexCount;
				indexCount++;
				vertexCount++;
				vertices[m_latitudeNum * 4 * i + vertexCount].position = XMFLOAT3(dHalf*cosf(i*angle + angle)*scaleAdjust, hHalf, dHalf*sinf(i*angle + angle)*scaleAdjust);
				vertices[m_latitudeNum * 4 * i + vertexCount].normal = XMFLOAT3(cosf(i*angle + angleHalf), tan(angleY), sinf(i*angle + angleHalf));
				// RightTop
				indices[indexCount] = m_latitudeNum * 4 * i + vertexCount;
				indexCount++;
				...
				vertices[m_latitudeNum * 4 * i + vertexCount].position = XMFLOAT3(dHalf*cosf(i*angle), -hHalf, dHalf*sinf(i*angle));
				vertices[m_latitudeNum * 4 * i + vertexCount].normal = XMFLOAT3(cosf(i*angle + angleHalf), tan(angleY), sinf(i*angle + angleHalf));
				// LeftBottom
				indices[indexCount] = m_latitudeNum * 4 * i + vertexCount;
				indexCount++;
				// RightTop
				indices[indexCount] = m_latitudeNum * 4 * i + vertexCount - 1;
				indexCount++;
				vertexCount++;
				vertices[m_latitudeNum * 4 * i + vertexCount].position = XMFLOAT3(dHalf*cosf(i*angle + angle), -hHalf, dHalf*sinf(i*angle + angle));
				vertices[m_latitudeNum * 4 * i + vertexCount].normal = XMFLOAT3(cosf(i*angle + angleHalf), tan(angleY), sinf(i*angle + angleHalf));
				// RightBottom
				indices[indexCount] = m_latitudeNum * 4 * i + vertexCount;
				indexCount++;
...
void Primitive3D::drawCube(ID3D11DeviceContext *a_pDeviceContext, const XMFLOAT3 &a_position, const XMFLOAT3 &a_size,const UINTCOLOR &a_blendColor, const CUSTOM3D* a_pCustom3D)
{
	...
	{
		//UP
		{ XMFLOAT3(x - wHalf, y + hHalf, z + dHalf), XMFLOAT3(0.0f, 1.0f, 0.0f) },
		{ XMFLOAT3(x + wHalf, y + hHalf, z + dHalf), XMFLOAT3(0.0f, 1.0f, 0.0f) },
		{ XMFLOAT3(x - wHalf, y + hHalf, z - dHalf), XMFLOAT3(0.0f, 1.0f, 0.0f) },
		{ XMFLOAT3(x + wHalf, y + hHalf, z - dHalf), XMFLOAT3(0.0f, 1.0f, 0.0f) },
		//UNDER
		...
		{ XMFLOAT3(x + wHalf, y - hHalf, z + dHalf), XMFLOAT3(0.0f, -1.0f, 0.0f) },
		//LEFT
		{ XMFLOAT3(x - wHalf, y + hHalf, z + dHalf), XMFLOAT3(-1.0f, 0.0f, 0.0f) },
		{ XMFLOAT3(x - wHalf, y + hHalf, z - dHalf), XMFLOAT3(-1.0f, 0.0f, 0.0f) },
		{ XMFLOAT3(x - wHalf, y - hHalf, z + dHalf), XMFLOAT3(-1.0f, 0.0f, 0.0f) },
		{ XMFLOAT3(x - wHalf, y - hHalf, z - dHalf), XMFLOAT3(-1.0f, 0.0f, 0.0f) },
		//RIGHT
		...
		{ XMFLOAT3(x + wHalf, y - hHalf, z + dHalf), XMFLOAT3(1.0f, 0.0f, 0.0f) },
		//FRONT
		{ XMFLOAT3(x - wHalf, y + hHalf, z - dHalf), XMFLOAT3(0.0f, 0.0f, -1.0f) },
		{ XMFLOAT3(x + wHalf, y + hHalf, z - dHalf), XMFLOAT3(0.0f, 0.0f, -1.0f) },
		{ XMFLOAT3(x - wHalf, y - hHalf, z - dHalf), XMFLOAT3(0.0f, 0.0f, -1.0f) },
		{ XMFLOAT3(x + wHalf, y - hHalf, z - dHalf), XMFLOAT3(0.0f, 0.0f, -1.0f) },
		//BACK
